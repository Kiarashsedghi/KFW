...
onebyte_np_t get_index_of_data_in_datas(kmc_t *kmci,onebyte_p_t *data_name){
    int i=0;
...
onebyte_np_t get_index_of_policy_in_policies(kmc_t *kmci,onebyte_p_t *policy_name){
...
onebyte_np_t check_policy_dependeny_on_data(kmc_t *kmci,onebyte_p_t *data_name){
    int i=0;
    int j=0;
    for(i=0;i<kmci->current_kfw_policies;i++){
...
onebyte_np_t get_index_of_policyint_in_egress(egress_policies_t *egressPolicies ,onebyte_p_t*interface_name){
    ...
    // we search in egress policies based on the interface_name not policy_name
...
onebyte_np_t get_index_of_policyint_in_ingress(ingress_policies_t *ingressPolicies ,onebyte_p_t *interface_name){
    ...
    // we search in ingress policies based on the interface_name not policy_name
    // Th reason is we want to find out whether we have defined a policy on the interface or not
    // if not , we create a new egress entry and if yes , we update that policy if it was changed
    // because we can only set one policy on each interface on each direction
...
//            printk(KERN_ERR "new data created%s %d\n",kmc_i.datas[0].name,kmc_i.datas[0].type);
                printk(KERN_INFO
                ">(%s)\n", kmc_i.datas[0].name);
                printk(KERN_INFO
                ">(%d)\n", kmc_i.datas[0].type);
...
//                    memcpy(kfwprepmss->payload, &kmc_i.datas[kmc_i.AUX_functions_returns], sizeof(data_t));
//                    printk(KERN_INFO
//                    "inja2\n");
